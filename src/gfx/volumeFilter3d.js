...
import MaterialBlur from './matblur';
...
export default class VolumeFilter3d{
    ...
    initRenderer(isRoiVolume, roiColors){
        ...
        this.cameraOrtho = new THREE.OrthographicCamera(this.xDim / -2, this.xDim / 2, this.yDim / 2, 0.1, 100);
   ...
   */
  createUpdatableVolumeTex(volume, isRoiVolume, roiColors, index) {
    this.isWebGL2 = 1;
    // this.arrPixels = volume.m_dataArray;//
    if(volume !== null){
      // console.log('>>>>>>>>>>>vol>>>>>>>>>',volume.m_slicesVolume.m_series[0]);
      const series = volume.m_slicesVolume.m_series[0];
      // const serie = series;
      const slices = series.m_slices;
      // const store = this.props;
      // const index = store.volumeIndex;
      // console.log('>>>>>>>>index>>>>>>>>',index)
      const slice = slices[index];
      this.arrPixels = slice.m_image;
    }
