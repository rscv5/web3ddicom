...
class ToolDistance{
    ...
    static textureToScreen(xTex, yTex, wScr, hScr, store){
        ...
        const xDim = dicom.m_boxSize.x;
        const yDim = dicom.m_boxSize.y;
        const zDim = dicom.m_zDim;
        const xPos = store.render2dxPos;
        const yPos = store.render2dyPos;
        // console.log('yPos, xPos', xPos, yPos)
        // x front-view
        vScr.x = ( xTex / xDim ) - xPos;
        vScr.y = ( yTex / yDim ) - yPos;
        vScr.x *= wScr;
        vScr.y *= hScr;
        return vScr;
    }
    ...
    static screenToTexture(xScr, yScr, wScr, hScr, store){
        ...
        // x front view
        vTex.x = Math.floor((xPos + xRel) * xDim);
        vTex.y = Math.floor((yPos + yRel) * yDim);
        return vTex;
    }
    ...
    onMouseUp(){ // omitted args: xScr, yScr, store
        ...
        if(numLines > 0){
            const objLastLine = this.m_lines[numLines - 1];
            objLastLine.distMm = this.getDistMm(objLastLine.vs, objLastLine.ve);
            // console.log('>>>>>>objLastLine.distMm>>>>>',objLastLine.distMm)
            const MIN_DIST_MM  = 0.00001;
            if(objLastLine.distMm <= MIN_DIST_MM){
                //remove last line
                this.m_lines.pop();
    ...
    render(ctx, store){
        ...
        for(let i = 0; i < numLines; i++){
            ...
            ctx.stroke();
            // draw text
            const xText = Math.floor((vs.x + ve.y) * 0.5);
